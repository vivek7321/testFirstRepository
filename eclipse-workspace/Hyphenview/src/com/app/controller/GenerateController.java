...
public class GenerateController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		LOGGER.info("In get method of GenerateController...");
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		LOGGER.info("In post method of GenerateController servlet...");
		...
		LOGGER.info("customQuery value in GenerateController --> " + customQuery);
		...
		LOGGER.info("reportId in servlet --> " +reportId);
		...
		LOGGER.info("chartName value in GenerateController --> " + chartName);
		...
		LOGGER.info("timeperiod value in GenerateController --> " + timeperiod);
		String theme = request.getParameter("theme");
		String newtheme = request.getParameter("newtheme");
		System.out.println("Theme in Param" +theme + "new" + newtheme);
		String startDate = request.getParameter("startDateGenerate");
		LOGGER.info("startDate value in GenerateController --> " + startDate);
		String endDate = request.getParameter("endDateGenerate");
		LOGGER.info("endDate value in GenerateController --> " + endDate);
		...
		//	ResultSet res = queryDao.executeQuery(email, customQuery, database, schema);
			...
			System.out.println("customQuery in GenerateController-->"+customQuery);
			...
			TableToPDF ttp = new TableToPDF();
			 try {
				 LOGGER.info("reportId--"+reportId + "StartDate-->"+startDate+"end date-->"+endDate);
				String fileNametoEmailBodyArray  = ttp.reportfileGenerate(reportId, startDate, endDate,"");
			} catch (Exception e1) {
				e1.printStackTrace();
			}
			 TableToXLS ttx = new TableToXLS();
			 try {
				String fileNametoEmailBodyArray  = ttx.reportfileGenerate(reportId, startDate, endDate, "");
			} catch (Exception e1) {
			...
			if(reportType.equalsIgnoreCase("chart")){
				...
				getServletContext().getRequestDispatcher("/GenerateChart.jsp").forward(request, response);
			}else if(reportType.equalsIgnoreCase("matrix")){
				request.setAttribute("dataset", res);
				getServletContext().getRequestDispatcher("/GenerateTable.jsp").forward(request, response);
			}
				...
				request.setAttribute("dataset", res);
				getServletContext().getRequestDispatcher("/GenerateBoxLayout.jsp").forward(request, response);
			}
		}
