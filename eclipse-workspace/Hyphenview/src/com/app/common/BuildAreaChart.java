...
public class BuildAreaChart {
	...
	public String configure(ReportTemplate reportTemplate, String chartId) throws Exception {
		...
		for (String key : tempMap.keySet()) {
			...
			if (null != tempMap.get(key).stream().map(Integer::parseInt).collect(Collectors.toList())
					&& tempMap.get(key).stream().map(Integer::parseInt).collect(Collectors.toList()).size() > 0) {
		...
		chartOptionsJs = "var " + chartId + " = bb.generate({\r\n" + "  title: {\r\n" + "    text: \""
				...
				"    },\r\n" + "onclick: function (d, i) { drilldowndata" + chartId
				+ "(d.name,this.categories()[d.index]); }" + "  },\r\n" + " axis: {\r\n" + "    x: {\r\n"
				+ "      type: \"category\",\r\n" + "      tick: {\r\n" + "        fit: false,\r\n"
				+ "        multiline: false,\r\n" + "        autorotate: true,\r\n" + "        rotate: 15,\r\n"
				+ "        culling: {max : 6}\r\n" + "      }\r\n" + "    },\r\n"
				+"	y: {\r\n" + 
